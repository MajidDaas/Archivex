...
# --- Helper: create a fresh Flow each time ---
def create_flow():
...
def callback():
    ...
    print(f"🔍 [CALLBACK] Session before validation: {dict(session)}")
...
def demo_login():
    if request.method == 'POST':
        ...
        print(f"🔍 [DEMO LOGIN] POST for email: '{email}'")
        ...
        if email and email in USER_ROLES:
            ...
            error_message = f"Demo login failed for '{email}'. Not in USER_ROLES."
